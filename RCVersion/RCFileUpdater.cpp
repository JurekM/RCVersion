...
bool RCFileUpdater::Load(const wchar_t* path)
{
	...
	if (0 != li.HighPart || 0x7FFFFFFF <= li.LowPart)
	...
	if (IsTextUnicode(bufferData, min(bytes, 256), &flags))
	{
		BYTE *temp = new BYTE[bytes + 2];
		ZeroMemory(temp, bytes + 2);
		CopyMemory(temp, bufferData, bytes);
		wchar_t* utf16 = reinterpret_cast<wchar_t*>(temp);
		int ccw = bytes / sizeof(wchar_t);
		if (0xFF == temp[0] && 0xFE == temp[1])
		{
			++utf16;
			--ccw;
		}
		WideCharToMultiByte(CP_UTF8, 0, utf16, ccw, bufferData, int(bufferSize), nullptr, nullptr);
		delete[] temp;
	}

	bufferChars = strlen(bufferData);
	return true;
}
...
bool RCFileUpdater::Save(const wchar_t* path)
{
	...
	if (INVALID_HANDLE_VALUE == hFile)
		return Error(GetLastError(), L"*** RCFileUpdater::Save: Cannot open output file", inputFile.c_str());

	AutoHClose ahc(hFile);

	DWORD bytes = 0;
	BOOL ok = WriteFile(hFile, bufferData, DWORD(bufferChars), &bytes, nullptr);
	if (!ok)
